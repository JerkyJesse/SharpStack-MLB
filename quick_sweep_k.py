"""Quick sweep of k_decay and surprise_k params."""
...
def _preload_data():
    global _games_df, _player_df, _fg_pitching, _altitude_bonus
...
def main():
    ...
    for param in TARGET_PARAMS:
        ...
        print(f"\n{'='*50}")
        print(f"Sweeping: {param}  (current={orig})")
        t0 = time.time()

        # Coarse
        candidates = np.arange(lo, hi + step * 0.01, step)
        print(f"  Coarse: {lo} to {hi} step {step} ({len(candidates)} pts)")
        ...
        print(f"  Fine: {sweep_lo:.2f} to {sweep_hi:.2f} step {fine_step:.4f} ({len(candidates2)} pts)")
        ...
        if best_val != orig:
            print(f"  >> IMPROVED {param}: {orig} -> {best_val}  ({baseline:.4f}% -> {best_acc:.4f}%)  [{elapsed:.1f}s]")
            improved[param] = (orig, best_val, best_acc - baseline)
        else:
